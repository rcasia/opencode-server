...
rm -rf /opt/opencode/compose.yaml /opt/opencode/Caddyfile /opt/opencode/switch.sh /opt/opencode/opencode.json /opt/opencode/nono-profile.json /opt/opencode/nono-version.json /opt/opencode/nono-cli-*.rpm /opt/opencode/nono-*.tar.gz /opt/opencode/nono-container /opt/opencode/nono /opt/opencode/host/app.sh /opt/opencode/host/monitoring.sh
# Pinned nono release (ADR-0025): the RPM (host) and musl tarball
# (container binary source) ride the S3 bundle, so boot never curls
# GitHub (ADR-0003). Toolchain per ADR-0024, so the install lives in
# this bootstrap and bumps replace the host. The container needs the
# musl build: the Alpine backend image is musl libc and the glibc
# binary fails there (missing ELF interpreter; gcompat lacks
# __res_init). Pins always reference upstream release artifacts;
...
NONO_TAR="nono-v0.76.0-x86_64-unknown-linux-musl.tar.gz"
