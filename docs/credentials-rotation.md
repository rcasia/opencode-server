...
|---|---|---|---|---|
| opencode web password (`opencode` user, machine-only behind SSO) | SSM `/opencode/server-password` | instance role, at boot → `/opt/opencode/opencode.env` (0600); derived `BASIC_AUTH` → `/opt/opencode/caddy.env` (0600) | `put-parameter --overwrite` + §1 | SSM command (seconds) or instance replacement |
| GitHub OAuth client secret (SSO human gate) | SSM `/opencode/github-oauth-secret` | instance role, at boot → `/opt/opencode/oauth2.env` (0600) | new OAuth secret + `put-parameter --overwrite` + §1b | SSM command (seconds) or instance replacement |
| oauth2-proxy cookie secret (32-byte base64url) | SSM `/opencode/oauth-cookie-secret` | instance role, at boot → `/opt/opencode/oauth2.env` (0600) | new random + `put-parameter --overwrite` + §1b | SSM command (seconds) or instance replacement |
...
5. Revoke: old PAT on GitHub; old OAuth client secret in the OAuth App
   ...
   completes (nothing to revoke — they only ever lived in SSM + the
